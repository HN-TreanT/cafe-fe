...
const DrawerPayment: React.FC<Props> = ({
  visible,
  setVisible,
  invoice_tables,
  invoice_details,
  ...
  const columns: ColumnsType<DataType> = [
    ...
    },
    {
      ...
      align: "center",
      render: (text: any) => (
        <div style={{ color: "#1677ff " }}>{convertPrice(text)}</div>
      ),
    },
  ];
  ...
  const onClickPayOrder = async () => {
    try {
      ...
      setDateRa(now);
      invoiceServices
        .paymentInvoice(selectedOrder?.id, selectedOrder?.price)
        .then((res: any) => {
          if (res.status) {
                ...
                <div className="info-payment">
                  ...
                  <div className="item-infor-payment">
                    ...
                    <span style={{ fontWeight: "500" }}>
                      {`${
                        selectedOrder?.price
                          ? convertPrice(value - selectedOrder?.price)
                          : 0
                      } `}
                    </span>
                  </div>
